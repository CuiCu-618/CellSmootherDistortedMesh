...
static double bytes[4] = {2 * sizeof(float) * N,
                          2 * sizeof(float) * N,
                          3 * sizeof(float) * N,
                          3 * sizeof(float) * N};
...
__global__ void
set_array(float *a, float value, int len)
{
...
__global__ void
STREAM_Copy(float *a, float *b, int len)
{
  int idx = threadIdx.x + blockIdx.x * blockDim.x;
  if (idx < len)
    b[idx] = a[idx];
...
__global__ void
STREAM_Scale(float *a, float *b, float scale, int len)
{
  int idx = threadIdx.x + blockIdx.x * blockDim.x;
  if (idx < len)
    b[idx] = scale * a[idx];
...
__global__ void
STREAM_Add(float *a, float *b, float *c, int len)
{
  int idx = threadIdx.x + blockIdx.x * blockDim.x;
  if (idx < len)
    c[idx] = a[idx] + b[idx];
...
__global__ void
STREAM_Triad(float *a, float *b, float *c, float scalar, int len)
{
  int idx = threadIdx.x + blockIdx.x * blockDim.x;
  if (idx < len)

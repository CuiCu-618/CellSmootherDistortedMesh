...
def plotfig(dim):
    ...
    Ax_DP = read_benchmark_data(target_Ax, files)

    precision = "SP"
    target_Ax = {f'Basic {precision}', f'BasicCell {precision}', f'ConflictFree {precision}'}
    Ax_SP = read_benchmark_data(target_Ax, files)
    ...
    fig.savefig(f'../build_DG/Figures/laplace_kernels_{dim}D.png', dpi=fig.dpi)
    fig.savefig(f'../build_DG/Figures/laplace_kernels_{dim}D.pdf', dpi=fig.dpi)
    # plt.show()

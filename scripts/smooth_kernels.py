...
smooth_DP_3d = read_benchmark_data(target_smooth, files3d, "smooth")

precision = "SP"
target_smooth = {f'Basic GLOBAL {precision}', f'Basic FUSED_L {precision}', f'ConflictFree ConflictFree {precision}'}
smooth_SP_3d = read_benchmark_data(target_smooth, files3d, "smooth")

# extract the array
Basic_ = smooth_DP_3d[:,0,:]
BasicCell_ = smooth_DP_3d[:,1,:]
CF_ = smooth_DP_3d[:,2,:]
...
# extract the array
Basic_ = smooth_SP_3d[:,0,:]
BasicCell_ = smooth_SP_3d[:,1,:]
CF_ = smooth_SP_3d[:,2,:]
...
fig.savefig('../build_DG/Figures/smooth_kernels.png', dpi=fig.dpi)
fig.savefig('../build_DG/Figures/smooth_kernels.pdf', dpi=fig.dpi)

...
for i in range(1, 8):
    ...
    for proc in range(1,5):
        local.append(f'../build_DG/BENCHMARK_PARALLEL_WEAK/Benchmark_poisson_3D_DGQ{i}_{proc}prcs_{proc}cell_ConflictFree_ConflictFree_ConflictFree_multiple_double.log')
    files.append(local)

Data_DP = []

target_Ax = {f'ConflictFree DP'}
for i in range(0,7):
    local = read_benchmark_data(target_Ax, files[i], "Ax")
...
fig.savefig('../build_DG/Figures/laplace_kernels_mpi_weak.png', dpi=fig.dpi)
fig.savefig('../build_DG/Figures/laplace_kernels_mpi_weak.pdf', dpi=fig.dpi)

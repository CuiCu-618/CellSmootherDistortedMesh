/**
 * @file cuda_mf.template.cuh
 * Created by Cu Cui on 2022/12/25.
 */

#ifndef CUDA_MF_TEMPLATE_CUH
#define CUDA_MF_TEMPLATE_CUH
...
namespace PSMF
{
  ...
  namespace internal
  {
      ...
      , hanging_nodes(dof_handler.get_triangulation())
    {
      cudaError_t error_code = cudaMemcpyToSymbol(
        CUDAWrappers::internal::constraint_weights,
    ...
    ReinitHelper<dim, Number>::setup_cell_arrays(const unsigned int color)
    {
      ...
      if (update_flags & update_JxW_values)
        JxW_host.resize(1 * padding_length);

      if (update_flags & update_gradients)
        inv_jacobian_host.resize(1 * padding_length * dim * dim);
      ...
      const std::shared_ptr<const Utilities::MPI::Partitioner> &partitioner)
    {
      ...
      memcpy(&local_to_global_host[cell_id],
             lexicographic_dof_indices.data(),
             1 * sizeof(types::global_dof_index));
      ...
      if (update_flags & update_JxW_values)
        {
          ...
          const unsigned int  offset            = 0 * padding_length;
          ...
          std::copy(&inv_jacobians[0][0][0],
                    &inv_jacobians[0][0][0] +
                      q_points_per_cell * sizeof(DerivativeForm<1, dim, dim>) /
                        sizeof(double),
                    &inv_jacobian_host[0 * padding_length * dim * dim]);
        }
    }
      ...
      if (update_flags & update_JxW_values)
        {
          if (data->parallelization_scheme ==
              MatrixFree<dim, Number>::parallel_over_elem)
            transpose_in_place(JxW_host, 1, padding_length);
      ...
      if (update_flags & update_gradients)
        {
          ...
          transpose_in_place(inv_jacobian_host, padding_length * 1, dim * dim);
    ...
    template <int dim, typename number, typename CellIterator>
    std::vector<types::global_dof_index>
    get_conflict_indices(const CellIterator              &cell,
  ...
  MatrixFree<dim, Number>::reinit(const Mapping<dim>              &mapping,
                                  ...
                                  const AdditionalData &additional_data)
  {
    IteratorFilters::LocallyOwnedLevelCell locally_owned_cell_filter;
    reinit(mapping,
           dof_handler,
           constraints,
           quad,
  ...
  {
    typename MatrixFree<dim, Number>::Data data_copy;
    if (q_points.size() > 0)
  ...
  {
    // First compute the size of n_cells, row_starts, kernel launch
    // parameters, and constrained_dofs
    ...
    for (unsigned int i = 0; i < n_colors; ++i)
      {
        bytes += n_cells[i] * padding_length * sizeof(unsigned int) +
                 1 * padding_length * dim * dim * sizeof(Number) +
                 1 * padding_length * sizeof(Number) +
                 0 * padding_length * sizeof(point_type) +
    ...
    if (mg_level != numbers::invalid_unsigned_int)
      {
        ...
        CellFilter begin(iterator_filter, dof_handler->begin_mg(mg_level));
        CellFilter end(iterator_filter, dof_handler->end_mg(mg_level));
                ...
                if (additional_data.overlap_communication_computation)
                  {
                    ...
                    for (const auto &cell :
                         dof_handler->get_triangulation()
                           .cell_iterators_on_level(mg_level))
                      if (cell->is_ghost())
        ...
        if (comm)
          {
            DoFTools::extract_locally_relevant_level_dofs(
              *dof_handler, mg_level, locally_relevant_dofs);
    ...
    if (n_constrained_dofs != 0)
      {
        ...
        if (partitioner)
        ...
        else
          {
            ...
            if (mg_level != numbers::invalid_unsigned_int)
              {
                n_local_dofs = dof_handler->n_dofs(mg_level);
              }
            else
              {
                n_local_dofs = dof_handler->n_dofs();
              }
            unsigned int i_constraint = 0;
            for (unsigned int i = 0; i < n_local_dofs; ++i)
              {

...
namespace PSMF
{
   ...
   * PDEs. If the same operation is applied to several components of a PDE
   * (e.g. a vector Laplace equation), they can be applied simultaneously with
   * one call (and often more efficiently). Defaults to 1
   *
  ...
  {
    local_to_global = data->local_to_global + 1 * cell_id;
    inv_jac         = data->inv_jacobian + padding_length * 0;
    JxW             = data->JxW + padding_length * 0;
    ...
    read_dof_values(const Number *src)
  {
    ...
    const types::global_dof_index src_idx = local_to_global[0];
    ...
    CUDAWrappers::internal::resolve_hanging_nodes<dim, fe_degree, false>(
    ...
    distribute_local_to_global(Number *dst) const
  {
    static_assert(n_components_ == 1, "This function only supports FE with one \
                  components");
    CUDAWrappers::internal::resolve_hanging_nodes<dim, fe_degree, true>(
      constraint_mask, values);

    const unsigned int idx = internal::compute_index<dim, n_q_points_1d>();

    const types::global_dof_index destination_idx = local_to_global[0];
    ...
    if (use_coloring)
      dst[global_idx] += values[idx];
    else
      atomicAdd(&dst[global_idx], values[idx]);
  }
    ...
    for (int d_1 = 0; d_1 < dim; ++d_1)
      {
        ...
        for (int d_2 = 0; d_2 < dim; ++d_2)
          tmp += inv_jacobian[1 * padding_length * (dim * d_2 + d_1)] *

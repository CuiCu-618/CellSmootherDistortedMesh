...
namespace PSMF
{
  ...
  public:
     ...
     * of degrees of freedom) or parallel_over_elem (parallelism at the level
     * of cells)
     */
    ...
    struct AdditionalData
    {
      ...
      /**
       * Parallelization scheme used, parallelization over degrees of freedom
       * or over cells.
       */
      ParallelizationScheme parallelization_scheme;
      /**
       * This flag is used to determine which quantities should be cached.
       * This class can cache data needed for gradient computations (inverse
       * Jacobians), Jacobian determinants (JxW), quadrature points as well as
       * data for Hessians (derivative of Jacobians). By default, only data
       * for gradients and Jacobian determinants times quadrature weights,
       * JxW, are cached. If quadrature points of second derivatives are
      ...
      /**
       *  Overlap MPI communications with computation. This requires
       * CUDA-aware MPI and use_coloring must be false.
       */
      bool overlap_communication_computation;

      /**
       * This option can be used to define whether we work on a certain level
       * of the mesh, and not the active cells. If set to invalid_unsigned_int
     ...
     * object (predicate) to loop over a subset of the active cells. When
     * using MPI, the predicate should filter out non locally owned cells.
     */
    ...
    /**
     * This method runs the loop over all cells and apply the local operation
     * on each element in parallel. This function is very similar to
     * cell_loop() but it uses a simpler functor.
     *
     * @p func needs to define
     * \code
     *  __device__ void operator()(
     *    const unsigned int                                          cell,
     *    const typename CUDAWrappers::MatrixFree<dim, Number>::Data
     * *gpu_data); static const unsigned int n_dofs_1d; static const unsigned
     * int n_local_dofs; static const unsigned int n_q_points; \endcode
     */
    ...
    /**
     * Initialize a serial vector. The size corresponds to the number of
     * degrees of freedom in the DoFHandler object.
     */
    void
     ...
     * locally owned degrees of freedom and the ghost elements correspond to
     * the (additional) locally relevant dofs.
     */
    void
    initialize_dof_vector(
     ...
     * given by the respective fields. If you want to have specific
     * information about these objects, you can query them with the respective
     * access functions. If you just want to initialize a (parallel) vector,
     * you should usually prefer this data structure as the data exchange
     * information can be reused from one vector to another.
     */
  ...
  private:
     ...
     * distributed_cell_loop() to exist for
    ...
    /**
     * Vector of pointer to the inverse Jacobian associated to the cells of
     * each color.
     */
   ...
   */
  template <int dim, typename Number>
  struct SharedDataCuda
  {
    /**
     * Constructor.
     */
    __device__
    SharedDataCuda(Number *vd, Number *gq[dim])
      : values(vd)
    {
  ...
  // This function determines the number of cells per block, possibly at
  // compile time (by virtue of being 'constexpr')
  ...
  cells_per_block_shmem(int dim, int fe_degree)
  {
    ...
    return dim==2 ? (fe_degree==1 ? CUDAWrappers::warp_size :    // 128
                     ...
                     1) :
           dim==3 ? (fe_degree==1 ? CUDAWrappers::warp_size/1 :  //  64
                     fe_degree==2 ? CUDAWrappers::warp_size/8 :  //  54
                     1) : 1;
    /* clang-format on */
  }
  ...
  template <int dim, typename Number>
  DataHost<dim, Number>
  copy_mf_data_to_host(const typename MatrixFree<dim, Number>::Data &data,
   ...
   * that the data in MatrixFree<dim, Number>::Data has been copied to the
   * host using copy_mf_data_to_host().
   *
   * @relates CUDAWrappers::MatrixFree
   */
  template <int dim, typename Number>
